...
public class Enemy extends Character {
    ...
    // We still keep a flag for patrol logic but it will be updated by comparing positions.
    private boolean goingRight = true;
    ArrayList<Projectiles> gunShot;

    // NEW FIELD: previous X position to track movement delta.
    private float prevX;
    ...
    public Enemy(Viewport viewport, float maxSpeed, float width, float height, float xspawn, float yspawn, int HP, int damageStat, Texture textureFile, ArrayList<Projectiles> bulletArray) {
        ...
        // Set the sprite’s origin to its center so flipping works correctly.
        CharacterSprite.setOriginCenter();
        // Initialize the previous X position.
        prevX = CharacterSprite.getX();
    }
